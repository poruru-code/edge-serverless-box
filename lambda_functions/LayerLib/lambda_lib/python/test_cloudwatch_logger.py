    ...
    try:
        with patch("urllib.request.urlopen") as mock_url:
            client.put_log_events(
                "test-group",
                "test-stream",
                [{"timestamp": 123000, "level": "INFO", "message": "hello test"}],
            )
            ...
            assert log_data["_time"] == 123.0
    finally:
        sys.stdout = sys.__stdout__


def test_cloudwatch_logger_local_behavior():
    # Force LOG_LEVEL to DEBUG for testing
    ...
    try:
        ...
        log_data = json.loads(output)
        assert log_data["level"] == "INFO"
        assert "_time" in log_data
        assert "_msg" in log_data
    finally:
        sys.stdout = sys.__stdout__


def test_cloudwatch_logger_dict_message():
    logger = CloudWatchLogger("test-user")
    captured_output = io.StringIO()
    sys.stdout = captured_output
    try:
        logger.info(lambda: {"action": "test", "status": "success"})
        output = captured_output.getvalue().strip()
        log_data = json.loads(output)
